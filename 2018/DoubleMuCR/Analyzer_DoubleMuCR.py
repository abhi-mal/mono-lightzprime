...
class DoubleMuCR_havingHPS(Module):
    def __init__(self,is_data,process,year):
        ...
        self.event_weights= dict.fromkeys(["met_trig","PU","nlo_ewk","nlo_qcd_ewk","nnlo_qcd","id_iso","hpsjet_sf","all"],1)
        self.debug_vars = {}
        self.getSFhists()
        pass
    ...
    def apply_id_iso_weights(self, event, event_weight,selected_muons):
            mu1 = selected_muons[0]; mu2 = selected_muons[1]
            #leading muon
            mu1_tightid_sf = self.get_sf_from_hist_2D(self.sfmap["muon_id_tight"],mu1.pt,abs(mu1.eta),"mu1_tight_id")
            mu1_looseid_sf = self.get_sf_from_hist_2D(self.sfmap["muon_id_loose"],mu1.pt,abs(mu1.eta),"mu1_loose_id")
            mu1_tightiso_sf= self.get_sf_from_hist_2D(self.sfmap["muon_iso_tight"],mu1.pt,abs(mu1.eta),"mu1_tight_iso")
            mu1_looseiso_sf= self.get_sf_from_hist_2D(self.sfmap["muon_iso_loose"],mu1.pt,abs(mu1.eta),"mu1_loose_iso")
            #subleading muon
            mu2_tightid_sf = self.get_sf_from_hist_2D(self.sfmap["muon_id_tight"],mu2.pt,abs(mu2.eta),"mu2_tight_id")
            mu2_looseid_sf = self.get_sf_from_hist_2D(self.sfmap["muon_id_loose"],mu2.pt,abs(mu2.eta),"mu2_loose_id")
            mu2_tightiso_sf= self.get_sf_from_hist_2D(self.sfmap["muon_iso_tight"],mu2.pt,abs(mu2.eta),"mu2_tight_iso")
            mu2_looseiso_sf= self.get_sf_from_hist_2D(self.sfmap["muon_iso_loose"],mu2.pt,abs(mu2.eta),"mu2_loose_iso")          
            self.event_weights["id_iso"] = 0.5*(mu1_tightid_sf * mu1_tightiso_sf * mu2_looseid_sf * mu2_looseiso_sf + mu2_tightid_sf * mu2_tightiso_sf * mu1_looseid_sf * mu1_looseiso_sf)
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        self.out.branch("hps_charge"  , "F");   self.out.branch("hps_phi"  , "F");            self.out.branch("hps_leadTkPtOverhpsPt"  , "F")
        self.out.branch("hps_pt"      , "F");   self.out.branch("hps_decaymode"  , "F");      self.out.branch("hps_photonsOutsideSignalCone"  , "F")
        self.out.branch("hps_mass"    , "F");   self.out.branch("hps_leadTkDeltaEta"  , "F"); self.out.branch("hps_leadTkDeltaPhi"  , "F")
        self.out.branch("hps_eta"     , "F");   self.out.branch("Associatedjet_index" , "F"); self.out.branch("Associatedfatjet_index", "F")
        # General
        self.out.branch("recoil" , "F") 
        self.out.branch("muon1_pt" , "F"); self.out.branch("muon1_eta" , "F"); self.out.branch("muon1_phi" , "F")
        self.out.branch("muon2_pt" , "F"); self.out.branch("muon2_eta" , "F"); self.out.branch("muon2_phi" , "F")
        self.out.branch("dimuon_pt" , "F"); self.out.branch("dimuon_eta" , "F"); self.out.branch("dimuon_phi" , "F")
        #event weights
        self.out.branch("evt_wt_met_trig" , "F"); self.out.branch("evt_wt_PU" , "F"); self.out.branch("evt_wt_nlo_ewk" , "F"); self.out.branch("evt_wt_nlo_qcd_ewk" , "F"); self.out.branch("evt_wt_nnlo_qcd" , "F"); self.out.branch("evt_wt_mu_id_iso" , "F"); self.out.branch("evt_wt_hpsjet_sf" , "F"); self.out.branch("evt_wt_all" , "F")        

        self.cutflow = ROOT.TH1F('cutflow', 'cutflow', 15, 0, 15)
        #event related
        ...
        self.cutflow.GetXaxis().SetBinLabel(7,"electron_veto")         
        self.cutflow.GetXaxis().SetBinLabel(8,"photon_veto") 
        #self.cutflow.GetXaxis().SetBinLabel(9,"bjet_veto")
        self.cutflow.GetXaxis().SetBinLabel(10-1,"atleast_1_hpsjet")
        self.cutflow.GetXaxis().SetBinLabel(11-1,"atleast_1_cleanhpsjet")         
        self.cutflow.GetXaxis().SetBinLabel(12-1,"dphi_HPSJet_MET") 
        #HPSJet related
        self.cutflow.GetXaxis().SetBinLabel(13-1,"HPSJet_ptetacuts")
        #self.cutflow.GetXaxis().SetBinLabel(14-1,"wjets_cleaning")
        #self.cutflow.GetXaxis().SetBinLabel(15-1,"zjets_cleaning") 
        self.cutflow.GetXaxis().SetBinLabel(16-3,"has_associatedak4Jet")
        self.cutflow.GetXaxis().SetBinLabel(17-3,"has_associatedfatJet") 
        self.cutflow.GetXaxis().SetBinLabel(18-3,"associatedfatJet_has2subjets")
        self.cutflow_events = dict.fromkeys({"total_events","passing_trigger","MET_filters","dphi_PFMET_TkMET","dMET_PF_Calo","muon_CRselection","electron_veto","photon_veto","atleast_1_hpsjet","atleast_1_cleanhpsjet","dphi_HPSJet_MET","HPSJet_ptetacuts","has_associatedak4Jet","has_associatedfatJet","associatedfatJet_has2subjets"},0)
        self.argmaxHPSJet = ROOT.TH1F('argmaxHPSJet', 'argmaxHPSJet', 11, 0, 11)
    ...
    def endFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        self.cutflow.SetBinContent(7,self.cutflow_events["electron_veto"])
        self.cutflow.SetBinContent(8,self.cutflow_events["photon_veto"]) 
        #self.cutflow.SetBinContent(9,self.cutflow_events["bjet_veto"]) 
        self.cutflow.SetBinContent(10-1,self.cutflow_events["atleast_1_hpsjet"]) 
        self.cutflow.SetBinContent(11-1,self.cutflow_events["atleast_1_cleanhpsjet"])
        self.cutflow.SetBinContent(12-1,self.cutflow_events["dphi_HPSJet_MET"])
        #HPSJet related
        self.cutflow.SetBinContent(13-1,self.cutflow_events["HPSJet_ptetacuts"])
        #self.cutflow.SetBinContent(14-1,self.cutflow_events["wjets_cleaning"])
        #self.cutflow.SetBinContent(15-1,self.cutflow_events["zjets_cleaning"])
        self.cutflow.SetBinContent(16-3,self.cutflow_events["has_associatedak4Jet"]) 
        self.cutflow.SetBinContent(17-3,self.cutflow_events["has_associatedfatJet"])
        self.cutflow.SetBinContent(18-3,self.cutflow_events["associatedfatJet_has2subjets"]) 
        outputFile.mkdir("hists"); outputFile.cd("hists") ; self.cutflow.Write() ; self.argmaxHPSJet.Write()
    ...
    def get_veto_electrons(self,lep): 
        select = False
        if(lep.pt > 10 and lep.cutBased >=1 and abs(lep.eta)<2.5): #cut-based ID Fall17 V2; only consider electrons passing veto Electron ID,pt,eta cuts
        #ele dz and dxy selection
                if( (abs(lep.eta) <= 1.479) and (abs(lep.dxy) < 0.05) and (abs(lep.dz) < 0.1 )): select = True
                elif( (abs(lep.eta) > 1.479) and (abs(lep.dxy) < 0.1) and (abs(lep.dz) < 0.2 )): select = True
        return select

    def pass_muon_CRselection(self,muons): # exactly 2 muons of opposite charge satisfying pt and eta requirements
        event_passes = False
        count_tight = 0; count_loose = 0
        selected_tight_muons = []; selected_loose_muons = []; dimuon = None
        for mu in muons: 
                if(mu.pt > 20 and abs(mu.eta)<2.4): #tight_cand
                        if(mu.tightId and mu.pfRelIso04_all<0.15): #tight id and iso requirements
                                count_tight +=1 ; selected_tight_muons.append(mu)
                elif (mu.pt > 10 and abs(mu.eta)<2.4): #loose_cand
                        if(mu.looseId and mu.pfRelIso04_all<0.25): #loose id and iso requirement
                            count_loose += 1; selected_loose_muons.append(mu)
        if(count_tight==0 or (count_tight+count_loose)!=2):   pass
        else:
                if(count_tight==2): muon1 = selected_tight_muons[0]; muon2 = selected_tight_muons[1]
                else:  muon1 = selected_tight_muons[0]; muon2 = selected_loose_muons[0]                 
                if(muon1.charge*muon2.charge>0) : pass    
                else: 
                        dimuon = muon1.p4() + muon2.p4()
                        Zmass_window = [60,120]
                        if (Zmass_window[0]<=dimuon.M()<=Zmass_window[1]): event_passes = True
        selected_muons = selected_tight_muons +  selected_loose_muons              
        return [event_passes,dimuon,selected_muons]
    ...
    def get_debug_vars(self,CRmuons,tag_num,event_weight):
        #print("Starting to fill debug vars")    
        mu_1 = CRmuons[0];mu_2= CRmuons[1]
        mu_1_tag = "muon1_%s"%str(tag_num); mu_2_tag = "muon2_%s"%str(tag_num)
        if mu_1_tag not in self.debug_vars.keys():
                self.debug_vars[mu_1_tag] = {
                        "pt": ROOT.TH1F('%s_pt'%mu_1_tag, '%s_pt'%mu_1_tag, 20, 0, 1000),
                        "eta":ROOT.TH1F('%s_eta'%mu_1_tag, '%s_eta'%mu_1_tag, 50, -5, 5),
                        "phi": ROOT.TH1F('%s_phi'%mu_1_tag, '%s_phi'%mu_1_tag, 40, -4, 4)
                }
                self.debug_vars[mu_2_tag] = {
                        "pt": ROOT.TH1F('%s_pt'%mu_2_tag, '%s_pt'%mu_2_tag, 20, 0, 1000),
                        "eta": ROOT.TH1F('%s_eta'%mu_2_tag,'%s_eta'%mu_2_tag, 50, -5, 5),
                        "phi": ROOT.TH1F('%s_phi'%mu_2_tag, '%s_phi'%mu_2_tag, 40, -4, 4)
                }  
        self.debug_vars[mu_1_tag]["pt"].Fill(mu_1.pt,event_weight); self.debug_vars[mu_1_tag]["eta"].Fill(mu_1.eta,event_weight); self.debug_vars[mu_1_tag]["phi"].Fill(mu_1.phi,event_weight)
        self.debug_vars[mu_2_tag]["pt"].Fill(mu_2.pt,event_weight); self.debug_vars[mu_2_tag]["eta"].Fill(mu_2.eta,event_weight); self.debug_vars[mu_2_tag]["phi"].Fill(mu_2.phi,event_weight)
        #print("done filling debug vars")
        return

    def fill_output_branches(self,HPSJet,recoil,selected_muons,dimuon,Associatedjet_index,Associatedfatjet_index):
        #HPSJets(Highest Pt tau object)
        self.out.fillBranch("hps_charge"  ,HPSJet.charge);   self.out.fillBranch("hps_phi"  ,HPSJet.phi);                       self.out.fillBranch("hps_leadTkPtOverhpsPt"  ,HPSJet.leadTkPtOverTauPt) # HPSobject is the highest pt tau in event..
        self.out.fillBranch("hps_pt"      ,HPSJet.pt);       self.out.fillBranch("hps_decaymode"  ,HPSJet.decayMode);           self.out.fillBranch("hps_photonsOutsideSignalCone"  ,HPSJet.photonsOutsideSignalCone)
        self.out.fillBranch("hps_mass"    ,HPSJet.mass);     self.out.fillBranch("hps_leadTkDeltaEta"  ,HPSJet.leadTkDeltaEta); self.out.fillBranch("hps_leadTkDeltaPhi"  ,HPSJet.leadTkDeltaPhi)
        self.out.fillBranch("hps_eta"     ,HPSJet.eta);      self.out.fillBranch("Associatedjet_index" , Associatedjet_index); self.out.fillBranch("Associatedfatjet_index", Associatedfatjet_index)
        # General
        self.out.fillBranch("recoil"  ,recoil)
        mu1_pt,mu1_eta,mu1_phi = selected_muons[0].pt,selected_muons[0].eta,selected_muons[0].phi
        mu2_pt,mu2_eta,mu2_phi = selected_muons[1].pt,selected_muons[1].eta,selected_muons[1].phi
        dimuon_pt,dimuon_eta,dimuon_phi = dimuon.Pt(),dimuon.Eta(),dimuon.Phi()
        ...
        self.out.fillBranch("muon1_pt" , mu1_pt); self.out.fillBranch("muon1_eta" , mu1_eta); self.out.fillBranch("muon1_phi" , mu1_phi)
        self.out.fillBranch("muon2_pt" , mu2_pt); self.out.fillBranch("muon2_eta" , mu2_eta); self.out.fillBranch("muon2_phi" , mu2_phi)
        self.out.fillBranch("dimuon_pt" , dimuon_pt); self.out.fillBranch("dimuon_eta" , dimuon_eta); self.out.fillBranch("dimuon_phi" , dimuon_phi)
        #file_out.write("Finished writing Muon pt,eta variables\n")
        # event weights
        if not(self.is_data):
                #for key,value in self.event_weights.items(): print("%s : %s \n"%(key,value))
                self.out.fillBranch("evt_wt_met_trig" ,self.event_weights["met_trig"]); self.out.fillBranch("evt_wt_PU" ,self.event_weights["PU"]); self.out.fillBranch("evt_wt_nlo_ewk" ,self.event_weights["nlo_ewk"]); self.out.fillBranch("evt_wt_nlo_qcd_ewk" ,self.event_weights["nlo_qcd_ewk"])
    ...
    def analyze(self, event):
        ...
        if not(self.is_data):  
                event_weight = self.apply_pu_weights(event,event_weight)
                if self.processname in ["Z1Jets","Z2Jets","WJets","GJets","DY1Jets","DY2Jets"]: event_weight = self.apply_kfactors(event,event_weight)
        self.cutflow_events["total_events"] += 1*event_weight
        ...
        pass_CR_selection,dimuon,selected_muons = self.pass_muon_CRselection(muons)
        ...
        event.recoil = (dimuon + met_4vec).Pt() # vector sum
        if not(self.is_data):  
                event_weight = self.apply_trigger_sf(event,event_weight)
                event_weight = self.apply_id_iso_weights(event,event_weight,selected_muons)
        # fixing the previous cutflow values because recoil wasn't available earlier FIXME
        self.cutflow_events["muon_CRselection"] += 1*event_weight; self.get_debug_vars(selected_muons,6,event_weight)

        electrons = Collection(event, "Electron")
        veto_electrons = filter(self.get_veto_electrons, electrons) 
        if (len(veto_electrons)>0): return to_return 
        self.cutflow_events["electron_veto"] += 1*event_weight; self.get_debug_vars(selected_muons,7,event_weight)

        photons = Collection(event, "Photon")
        veto_photons = filter(self.get_veto_photons, photons)
        if (len(veto_photons)>0): return to_return 
        self.cutflow_events["photon_veto"] += 1*event_weight; self.get_debug_vars(selected_muons,8,event_weight)

        jets = Collection(event, "Jet")
        #loose_bjets = filter(self.get_loose_bjets, jets)
        #if (len(loose_bjets)>0): return to_return 
        #self.cutflow_events["bjet_veto"] += 1*event_weight; self.get_debug_vars(selected_muons,9,event_weight)

        hpsjets = Collection(event, "Tau")
        if(len(hpsjets)<=0): return to_return
        # can't apply weight here because the hps object is not yet selected, so applying it below
        self.cutflow_events["atleast_1_hpsjet"] += 1*event_weight; self.get_debug_vars(selected_muons,10-1,event_weight)

        clean_hps_jets = self.get_clean_hps_jets(hpsjets,veto_electrons,selected_muons,veto_photons)
        if(len(clean_hps_jets)<=0): return to_return 
        self.cutflow_events["atleast_1_cleanhpsjet"] += 1*event_weight; self.get_debug_vars(selected_muons,11-1,event_weight)        
        HPSJet = clean_hps_jets[0] #Choosing highest Pt HPSJet 
        max_index = self.get_argmax(clean_hps_jets)
        #if not(max_index ==0):file_out.write("HPS max pt index = %s"%max_index)
        self.argmaxHPSJet.Fill(max_index)     
        if not(self.is_data):  event_weight = self.apply_hpsjet_sf(HPSJet,event_weight)

        dphi_HPSJet_MET = abs(HPSJet.phi-event.MET_phi)
        if not(dphi_HPSJet_MET > 0.5 ): return to_return
        self.cutflow_events["dphi_HPSJet_MET"] += 1*event_weight; self.get_debug_vars(selected_muons,12-1,event_weight)

        #HPSJet cleaning
        if not((HPSJet.eta < 2.4) and (HPSJet.pt > 100)): return to_return
        self.cutflow_events["HPSJet_ptetacuts"] += 1*event_weight; self.get_debug_vars(selected_muons,13-1,event_weight)
        ...
        #self.cutflow_events["wjets_cleaning"] += 1*event_weight; self.get_debug_vars(selected_muons,14-1,event_weight)
        ...
        #self.cutflow_events["zjets_cleaning"] += 1*event_weight; self.get_debug_vars(selected_muons,15-1,event_weight)   

        HPSJet_jetIdx = HPSJet.jetIdx
        if not(0<=HPSJet_jetIdx<len(jets)):  return to_return
        Associatedak4Jet = jets[HPSJet_jetIdx]
        if(Associatedak4Jet.jetId <6):  return to_return #Jet ID flags bit1 is loose (always false in 2017 since it does not exist), bit2 is tight, bit3 is tightLepVeto
        self.cutflow_events["has_associatedak4Jet"] += 1*event_weight; self.get_debug_vars(selected_muons,16-3,event_weight) 

        fatjets = Collection(event, "FatJet")
        fatjet_idx = self.match_hps_fatjetarray(HPSJet.eta,HPSJet.phi,fatjets)# HPSJet eta and phi are for a single HPSJet, fatjets is collection  
        if not(0<=fatjet_idx<len(fatjets)): return to_return
        AssociatedFatJet = fatjets[fatjet_idx]
        self.cutflow_events["has_associatedfatJet"] += 1*event_weight; self.get_debug_vars(selected_muons,17-3,event_weight)
        ...
        self.fill_output_branches(HPSJet,event.recoil,selected_muons,event.dimuon,HPSJet_jetIdx,fatjet_idx)
...
parser = argparse.ArgumentParser(
    "Analyzing the nanoAOD tau-objects(called HPS Jets) and associated jets for DoubleMuCR")
parser.add_argument(
    "inputnanoAODFile",
    action="store",
    nargs='?',
...
branchsel_in = "./keep_branches_input_havingHPS.txt"
branchsel_out = "./keep_branches_output_havingHPS.txt"
...
p = PostProcessor(".", files, prefetch=True, cut=preselection, branchsel=branchsel_in, outputbranchsel= branchsel_out, modules=[
                  HPSJetCorrs2018ReReco(),DoubleMuCR_havingHPS(args.isdata,args.processname,args.year),ak4JetAndMetCorrections2018(),ak8JetCorrections2018()], postfix=args.postfix_outputFilename,noOut=False,maxEntries=None,jsonInput=jsonInput)
p.run()
